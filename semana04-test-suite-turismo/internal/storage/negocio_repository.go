...
func (r *NegocioMemoria) Guardar(n models.Negocio) error {
	if strings.TrimSpace(n.Nombre) == "" {						//  1. Nombre no puede estar vacío (después de TrimSpace) → ErrDatosInvalidos
		return errs.ErrDatosInvalidos
	}
	if !models.TiposValidos[n.Tipo] { 							//  2. Tipo debe estar en models.TiposValidos → ErrDatosInvalidos
		return errs.ErrDatosInvalidos
	}
	if len(n.IdiomasHablados) == 0 {							//  3. IdiomasHablados no puede estar vacío → ErrDatosInvalidos
		return errs.ErrDatosInvalidos
	}
	for _, idioma := range n.IdiomasHablados {					//  4. Cada idioma debe estar en models.IdiomasValidos → ErrDatosInvalidos
		if !models.IdiomasValidos[idioma] {
			return errs.ErrDatosInvalidos
		}
	}
	if _, existe := r.datos[n.ID]; existe {						//  5. ID no puede estar ya ocupado → ErrYaExiste
