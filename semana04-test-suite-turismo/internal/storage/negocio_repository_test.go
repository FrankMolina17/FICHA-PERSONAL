...
func TestBuscarPorID_NegocioExiste(t *testing.T) { //	Asegura que el lógica funciona cuando todo está bien.
	repo := NewNegocioMemoria()
...
func TestNegocioMemoria_BuscarPorID_Errores(t *testing.T) { //Asegura que el código es seguro y predecible cuando algo falla.
	...
	for _, c := range casos {
		t.Run(c.nombre, func(t *testing.T) {
			...
			if !errors.Is(err, c.errEsperado) {
				t.Fatalf("esperaba error %v, obtuvo %v", c.errEsperado, err)
			}
			if got.ID != 0 || got.Nombre != "" || got.Tipo != "" || got.Ciudad != "" || len(got.IdiomasHablados) != 0 || got.Activo {
				t.Errorf("esperaba Negocio vacío en error, obtuvo %+v", got)
			}
		})
	}
}
