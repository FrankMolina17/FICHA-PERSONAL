...
func TestCheckInMemoria_BuscarPorTurista(t *testing.T) {
	casos := []struct {
	...
	}{
		{
			...
			preparar: func(t *testing.T, checkins *CheckInMemoria) { // preparar
				t.Helper()
				a := models.CheckIn{ID: 1, TuristaID: 1, NegocioID: 1, Fecha: "2026-04-10", Calificacion: 5} //primer check-in
				b := models.CheckIn{ID: 2, TuristaID: 1, NegocioID: 1, Fecha: "2026-04-11", Calificacion: 4} //segundo check-in
			...
			turistaID:    1, //Este es el caso 1: Cuando el ID del turista existe
			errEsperado:  nil,
			esperadosLen: 2,
			esperadosIDs: map[int]struct{}{1: {}, 2: {}},	
		},
		{
			nombre:       "turista existe pero sin check-ins → slice vacío, sin error", //Este es el caso 2: Cuando el turista existe pero no tiene check-ins
			preparar:     nil,
			turistaID:    1,
			errEsperado:  nil,
			esperadosLen: 0,
		...
		{
			nombre:       "turista inexistente → ErrNoEncontrado", //Este es el caso 3: Cuando el ID del turista no existe
			preparar:     nil,
			turistaID:    999,
			errEsperado:  errs.ErrNoEncontrado,
			esperadosLen: -1,
		...
		{
			nombre:       "ID turista negativo → ErrDatosInvalidos",  	//Este es el caso 4: Cuando el ID del turista es negativo
			preparar:     nil,
			turistaID:    -1,
			errEsperado:  errs.ErrDatosInvalidos,
			esperadosLen: -1,
	...
	for _, c := range casos {
		t.Run(c.nombre, func(t *testing.T) { // Este es el caso de prueba
			_, _, checkins := setupRepos(t)
			if c.preparar != nil {
				c.preparar(t, checkins)
			}
			visitas, err := checkins.BuscarPorTurista(c.turistaID) // Este busca los check-ins del turista
...
func TestCheckInMemoria_Listar(t *testing.T) {   //lista todos los check-ins
	...
	t.Run("varios check-ins listados (sin depender del orden)", func(t *testing.T) { //genera la lista de check-ins
		_, _, checkins := setupRepos(t)
